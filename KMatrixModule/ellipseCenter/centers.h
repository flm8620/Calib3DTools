...
template<typename T>
T model_luminance_alternative(int x, int y, int wi, int he, libNumerics::vector<T> P, bool clr)
{
    T err = 0, lum = 0;
    T dist = -512;
    T l1 = P[0], l2 = P[1], theta = P[2], tu = P[3], tv = P[4], r1 = P[5], p = P[6], r2 = P[7],
      vh = P[8], vb = P[9], c = P[10];
...
template<typename T>
void jacobian_alternative(libNumerics::matrix<T> &J, const libNumerics::vector<T> &P, int wi,
                          int he, bool clr, int xbegin, int xend, int ybegin, int yend)
{
    T l1 = P[0], l2 = P[1], theta = P[2], tu = P[3], tv = P[4], r1 = P[5], p = P[6], r2 = P[7],
      vh = P[8], vb = P[9], c = P[10];
    int idx = 0;
...
template<typename T>
T model_luminance(int x, int y, int wi, int he, libNumerics::vector<T> P, bool clr)
{
    T err = 0, lum = 0;
    T dist = -512;
    T l1 = P[0], l2 = P[1], theta = P[2], tu = P[3], tv = P[4], r1 = P[5], p = P[6], r2 = P[7],
...
template<typename T>
void jacobian(libNumerics::matrix<T> &J, const libNumerics::vector<T> &P, int wi, int he, bool clr,
            ...
            if (d != -512) {
                ...
                if (d < x20 && d > x10) {
                    ...
                    if (d <= x21 && d >= x11) {
                        ...
                        J(idx,
                          7) = 1 - 1/(sqrt(r2*r2 - (d-x20)*(d-x20))) * (r2 + (d-x20) * dX20dR2);
...
{
public:
    LMTacheC(ImageGray<double> &Im, T cx, T cy, T delta, bool tache_color, int img_width,
             int img_height)
    {
        clr = tache_color;
        wi = img_width;
