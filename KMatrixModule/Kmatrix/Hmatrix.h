...
T evaluateHomography(matrix<T> &H, std::vector<matrix<T> > &S, matrix<T> &H0CS)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        matrix<T> cs = matrix<T>::zeros(3, 1);
        cs(0, 0) = csx;
        cs(1, 0) = csy;
        cs(2, 0) = 1;
        matrix<T> hcs = H*cs;
...
template<typename T>
bool solveHomography(matrix<T> &x1_, matrix<T> &x2_, matrix<T> &Hresult)
{
    ...
    int n = x1_.ncol();
    if (n < 4) {
        libMsg::cout<<"solveHomography: The number of circles is less than 4, impossible to continue."
                    <<libMsg::endl;
        return false;
    }
...
matrix<T> groundH(matrix<T> &K, T thetax, T thetay, T thetaz, T tx, T ty, T tz)
{
    ...
    Rx(0, 0) = 1;
    Rx(1, 1) = std::cos(thetax);
    Rx(1, 2) = -std::sin(thetax);
    Rx(2, 1) = std::sin(thetax);
    Rx(2, 2) = std::cos(thetax);

    Ry(0, 0) = std::cos(thetay);
    Ry(0, 2) = std::sin(thetay);
    Ry(1, 1) = 1;
    Ry(2, 0) = -std::sin(thetay);
    Ry(2, 2) = std::cos(thetay);

    Rz(0, 0) = std::cos(thetaz);
    Rz(0, 1) = -std::sin(thetaz);
    Rz(1, 0) = std::sin(thetaz);
    Rz(1, 1) = std::cos(thetaz);
    Rz(2, 2) = 1;
...
template<typename T>
class LMhomography2 : public MinLM<T>
{
public:
    LMhomography2(std::vector<matrix<T> > &s, matrix<T> &uv)
    {
        S = s;
        UV = uv;
    }
...
public:
    ...
    virtual void modelJacobian(const vector<T> &P, matrix<T> &J) const
    {
        ...
        int ncircle = J.nrow();
        for (int i = 0; i < ncircle; i++) {
            jac_inv_maple(P, S[i], UV(0, i), UV(1, i), J(i, 0), J(i, 1), J(i, 2),
                          J(i, 3), J(i, 4), J(i, 5), J(i, 6), J(i, 7), J(i, 8));
        }
    }
};
template<typename T>
class LMhomography3 : public MinLM<T>
{
public:
    LMhomography3(std::vector<matrix<T> > &s, matrix<T> &uv)
    {
        S = s;
        UV = uv;
    }
...
public:
    ...
    virtual void modelJacobian(const vector<T> &P, matrix<T> &J) const
    {
        ...
        int ncircle = J.nrow();
        for (int i = 0; i < ncircle; i++) {
            jac_inv_maple_noOptimize(P, S[i], UV(0, i), UV(1, i), J(i, 0), J(i, 1), J(i, 2),
                                     J(i, 3), J(i, 4), J(i, 5), J(i, 6), J(i, 7), J(i, 8));
        }
    }
};
