...
void read_degree(FILE *pfile, int& degX, int& degY) {
    ...
    bool flagX = false, flagY = false, firstRead = true;
	while (!feof(pfile)) {
        ...
        //std::cout<<"Scanning file 1: sign : "<<sign<<std::endl;
		/* if sign "#" is met - it's a comment, we may skip it. */
		if (strcmp(sign, grid) != 0) {
            //std::cout<<"Not a comment : let's read what's next"<<std::endl;
			char mono1[5], mono2[5];
            char deg1[5], deg2[5];
            fscanf(pfile, "%lf", &coef);
            if(coef == 11) // Cf. les fichiers de poly : un 11 apparait au début et au milieu
                fscanf(pfile, "%lf", &coef);
            //std::cout<<"Scanning file : coeff : "<<coef<<std::endl;
			long currPos = ftell (pfile);
			fscanf(pfile, "%s", sign);
            //std::cout<<"Scanning file : sign : "<<sign<<std::endl;
            if (strcmp(sign, star) != 0) {
                //std::cout<<"Sign is not a star 1"<<std::endl;
				if (!feof(pfile)) {
                    //std::cout<<"As we're not a the end of the file 1..."<<std::endl;
					fseek(pfile, currPos, SEEK_SET);
					assert(strcmp(sign, plus) == 0 || strcmp(sign, minu) == 0 || strcmp(sign, grid) == 0); 	} }
            else {
                //std::cout<<"Sign IS a star : what's next?"<<std::endl;
                fscanf(pfile, "%s", mono1);
                fscanf(pfile, "%s", deg1);
                //std::cout<<"Scanning file : mono1 : "<<mono1<<std::endl;
                //std::cout<<"Scanning file : deg1 : "<<deg1<<std::endl;
                currPos = ftell (pfile);
				fscanf(pfile, "%s", sign);
                //std::cout<<"Scanning file : sign : "<<sign<<std::endl;
                if (strcmp(sign, star) != 0) {
                    //std::cout<<"Sign is not a star 2"<<std::endl;
					if (!feof(pfile)) {
                        //std::cout<<"As we're not a the end of the file 2..."<<std::endl;
						fseek(pfile, currPos, SEEK_SET);
                        assert(strcmp(sign, plus) == 0 || strcmp(sign, minu) == 0 || strcmp(sign, grid) == 0); 	}
				}

                else {
                    fscanf(pfile, "%s", mono2);
                    fscanf(pfile, "%s", deg2);
                    //std::cout<<"Scanning file : mono2 : "<<mono2<<std::endl;
                    //std::cout<<"Scanning file : deg2 : "<<deg2<<std::endl;
                }
            }
            ...
            int tmpdeg = std::max(atoi(deg1), atoi(deg2));
            //std::cout<<"Degre : "<<tmpdeg<<std::endl;
            ...
            if(feof(pfile)){
                //std::cout<<"Fin du fichier : c'est terminé"<<std::endl;
                break;
            }
		}
        else {
    ...
    std::cout<<"degY in function: "<<degY<<std::endl;
    degY=100;
    std::cout<<"degY in function2: "<<degY<<std::endl;
...
vector<T> read_poly(char* fname, int& degX, int& degY) {
    ...
    std::cout<<"degY out function: "<<degY<<std::endl;
    int sizex = (degX + 1) * (degX + 2) / 2;
	int sizey = (degY + 1) * (degY + 2) / 2;
