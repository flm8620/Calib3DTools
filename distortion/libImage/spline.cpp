...
bool prepare_spline_RGB(image_double_RGB &im, int order)
{
    ...
    for (int y = 0; y < im->ysize; y++) { // Filter on lines
        invspline1D(im->Rdata+y*im->xsize, 1, im->xsize, z, npoles);
        invspline1D(im->Gdata+y*im->xsize, 1, im->xsize, z, npoles);
        invspline1D(im->Bdata+y*im->xsize, 1, im->xsize, z, npoles);
    }
    for (int x = 0; x < im->xsize; x++) { // Filter on columns
        invspline1D(im->Rdata+x, 1*im->xsize, im->ysize, z, npoles);
        invspline1D(im->Gdata+x, 1*im->xsize, im->ysize, z, npoles);
        invspline1D(im->Bdata+x, 1*im->xsize, im->ysize, z, npoles);
    }
    return true;
}
...
bool interpolate_spline(image_double &im, int order, double x, double y, double &out,
                        double paramKeys)
{
    double cx[12], cy[12];
...
bool interpolate_spline_RGB(image_double_RGB &im, int order, double x, double y,
                            pixel_double_RGB &out, double paramKeys)
{
    double cx[12], cy[12];
    ...
    if (order == 0) { /* zero order interpolation (pixel replication) */
        int xi = (int)floor((double)x);
        int yi = (int)floor((double)y);
        bInside = valid_image_double_RGB(im, xi, yi);// im.valid(xi, yi);
        if (bInside) {
            out.R = im->Rdata[xi+yi*im->xsize];
            out.G = im->Gdata[xi+yi*im->xsize];
            out.B = im->Bdata[xi+yi*im->xsize];
        }
    } else { /* higher order interpolations */
        bInside = (x >= 0.0f && x <= (double)im->xsize && y >= 0.0f && y <= (double)im->ysize);
        if (bInside) {
            ...
            int n1 = 1-n2;
            /* this test saves computation time */
            if (valid_image_double_RGB(im, xi+n1,
                                       yi+n1) && valid_image_double_RGB(im, xi+n2, yi+n2)) {
                out.R = 0.;
                out.G = 0.;
                out.B = 0.;
                for (int dy = n1; dy <= n2; dy++) {
                    int adrs = (xi+n1) + (yi+dy) * im->xsize;
                    for (int dx = n1; dx <= n2; dx++) {
                        double red = im->Rdata[adrs];
                        double green = im->Gdata[adrs];
                        double blue = im->Bdata[adrs];
                        out.R += cy[n2-dy]*cx[n2-dx] * red;
                        out.G += cy[n2-dy]*cx[n2-dx] * green;
                        out.B += cy[n2-dy]*cx[n2-dx] * blue;
                        adrs++;
                    }
                }
            } else {
