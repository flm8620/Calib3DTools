...
bool interpolate_spline_RGB(image_double_RGB &im, int order, double x, double y, pixel_double_RGB &out, double paramKeys)
{
    double  cx[12],cy[12];
    ...
    if(order == 0) { /* zero order interpolation (pixel replication) */
        ...
        if(bInside) {
            ...
            int n1 = 1-n2;
            /* this test saves computation time */
            if(valid_image_double_RGB(im, xi+n1, yi+n1) && valid_image_double_RGB(im, xi+n2,yi+n2)) {
                out.R=out.G=out.B=0.0;
                for(int dy = n1; dy <= n2; dy++) {
                    int adrs = (xi+n1) + (yi+dy) * im->xsize;
                    for(int dx = n1; dx <= n2; dx++) {
                        out.R += cy[n2-dy]*cx[n2-dx] * im->Rdata[adrs];
                        out.G += cy[n2-dy]*cx[n2-dx] * im->Gdata[adrs];
                        out.B += cy[n2-dy]*cx[n2-dx] * im->Bdata[adrs];
                        adrs++;
                    }
                }
