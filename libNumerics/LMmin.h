...
public:
    MinLM() : iterations(0),
        relativeTol(DEFAULT_RELATIVE_TOL),
        lambdaInit(DEFAULT_LAMBDA_INIT),
        lambdaFact(DEFAULT_LAMBDA_FACT)
    {
    }
...
//            }
//        }
//            std::cout<<"Iter:"<<this->iterations<<std::endl;
...
//    }
    ...
    T minimize(vector<T> &P, const vector<T> &yData, T targetRMSE = 0.1, int maxIters = 300)
    {
        T sqErrorMax_n = targetRMSE * targetRMSE * yData.nrow();
        vector<T> yModel(yData.nrow());
        modelData(P, yModel);
        vector<T> E(yData-yModel);
        ...
        vector<T> B = Jt*E;
        compress(JtJ, B);
        T sqRelativeTolUp = (1+relativeTol)*(1+relativeTol);
        T sqRelativeTolDown = (1-relativeTol)*(1-relativeTol);
        T lambda = lambdaInit;
        for (iterations = 0; iterations < maxIters &&sqError_n > sqErrorMax_n; iterations++) {
            matrix<T> H(JtJ);
            for (int i = 0; i < H.nrow(); i++)
                H(i, i) *= 1 + lambda;
            vector<T> dP(P.nrow());
            ...
            modelData(tryP, yModel);
            E = yData - yModel;
            T trySqError_n = E.qnorm();
            T sqRatio = trySqError_n/sqError_n;
            if (sqRatio <= sqRelativeTolUp && sqRatio >= sqRelativeTolDown)
                break;
            if (trySqError_n > sqError_n) {
                lambda *= lambdaFact;
            } else {
                lambda /= lambdaFact;
                sqError_n = trySqError_n;
                P = tryP;
                modelJacobian(P, J);
                Jt = J.t();
                JtJ = Jt*J;
        ...
        std::cout<<"Iter:"<<this->iterations<<std::endl;
        return std::sqrt(sqError_n/E.size());
    }
